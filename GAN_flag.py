...
def load_images_flat(root_dir,
    ...
    # returns a list of numpy arrays of flag data
    arrays = []
    if recursive:
...
class Discriminator:
    def __init__(self, img_channels=1, H=28, W=28):
...
def train(
    ...
    root = r"flags"
    dataset = load_images_flat(root, to_rgb=True, target_size=(28,28)).astype(np.float32)
    if dataset.ndim == 3:
